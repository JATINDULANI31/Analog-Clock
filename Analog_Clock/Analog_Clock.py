...
from functions import get_ticks, draw_time
...
for i in range(len(hours_init)):
	if i % 5 == 0:
		cv2.line(image, hours_init[i], hours_dest[i], COLORS['gray'], 3)
	else:
		cv2.circle(image, hours_init[i], 5, COLORS['white'], -1)

#Draw some decorations on the watch
cv2.circle(image, (320,320), RADIUS+10, COLORS['dark_gray'], 2)
cv2.putText(image, "ROLEX", (215,230), cv2.FONT_HERSHEY_SCRIPT_COMPLEX, 2, COLORS['cyan'], 1, cv2.LINE_AA)
