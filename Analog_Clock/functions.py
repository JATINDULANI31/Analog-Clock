...
def draw_time(image):
	...
	cv2.line(image, CENTER, (second_x, second_y), COLORS['blue'], 2, cv2.LINE_AA)
    # pts = np.array([(second_x-1,second_y-1), (second_x,second_y), (second_x+1,second_y-1)], np.int32)
	...
	cv2.arrowedLine(image, CENTER, (minute_x, minute_y), COLORS['blue'], 5, cv2.LINE_AA)
	...
	cv2.arrowedLine(image, CENTER, (hour_x, hour_y), COLORS['blue'], 6, cv2.LINE_AA)

	cv2.circle(image, CENTER, 5, COLORS['dark_gray'], -1)

	time = getDigitalTime(int(hour),minute,second)

	cv2.putText(image,time, (200,390), cv2.FONT_HERSHEY_DUPLEX, 1.6, COLORS['cyan'], 1, cv2.LINE_AA)
